...
def password(update, context):
    ...
    user_password = cur.execute('''SELECT password FROM User WHERE username = (?)''', ((user_data['username']),)).fetchone()[0]
...
def check(update, user_data):
    ...
    conn = sqlite3.connect("../../database/db.sqlite3")
    cur = conn.cursor()
    idTelegram_stored = cur.execute('''SELECT idTelegram FROM User WHERE username = (?)''', ((user_data['username']),)).fetchone()[0]
    if idTelegram_stored == update.message.from_user.id:
        ...
        cur.execute('''UPDATE User SET idTelegram = (?) WHERE username=(?)''', ((update.message.from_user.id),(user_data['username']),))
